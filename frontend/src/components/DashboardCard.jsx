const DashboardCard = ({
  ...
  accent = "bg-temple-50 text-temple-600 dark:bg-temple-500/20 dark:text-temple-400",
  hideTrend = false,
}) => {
  ...
  return (
    <div className="relative overflow-hidden bg-white/60 dark:bg-[#1e293b]/70 backdrop-blur-xl rounded-2xl p-5 border border-white/40 dark:border-white/5 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 group">
      ...
      <div className="relative flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          {!hideTrend && trend != null && trend !== "" && (
            <div className={`px-2.5 py-1 rounded-full text-xs font-bold bg-white dark:bg-[#0f172a] shadow-sm ${trendTextColor}`}>
              {trendPrefix} {trend}
            </div>
          )}
        </div>
