...
const CashierAccounts = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [metricsRes, txRes, pendingRes] = await Promise.all([
        getDashboardMetrics(),
        getTransactions(),
        getTransactions({ status: "Pending Approval" })
      ]);
            ...
            data.map((t) => (
              <tr key={t._id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${t.transactionType === "Credit" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                    {t.transactionType}
                  </span>
                </td>
                <td className="px-6 py-4">
          ...
          <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-green-50 border border-green-200 p-6 rounded-2xl">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h4 className="text-slate-600 font-medium">Today's Income</h4>
                      </div>
                    ...
                    <div className="bg-red-50 border border-red-200 p-6 rounded-2xl">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h4 className="text-slate-600 font-medium">Today's Expense</h4>
                      </div>
                      <p className="text-3xl font-bold text-slate-800">₹{metrics.todayExpense?.toFixed(2) || '0.00'}</p>
                    </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 p-6 rounded-2xl">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h4 className="text-slate-600 font-medium">Cash in Hand (Est.)</h4>
                      </div>
                      <p className="text-3xl font-bold text-slate-800">₹{metrics.cashInHand?.toFixed(2) || '0.00'}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {activeTab === "pending" && (
              <div className="space-y-4">
                ...
                {renderTransactionsTable(pendingPayments)}
              </div>
            )}
          </>
        )}
